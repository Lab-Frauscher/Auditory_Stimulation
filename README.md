...
> **Prerequisites:** 
> - Raw EEG demo recordings (`.edf`) use **Git Large File Storage (Git LFS)**. Ensure Git LFS is installed prior to cloning.
> - Automated slow-wave detection requires SSH access to the private GitLab repository (`anphy_duke/lab-tools/slow-wave-detector`)[cite: 1].
...
Clone and install the private slow-wave detector package into your active environment:

```bash
# Clone slow-wave-detector repository via SSH
git clone git@gitlab.com:anphy_duke/lab-tools/slow-wave-detector.git
...
*Typical installation time: < 5 minutes (including Git LFS pull and detector setup).*
