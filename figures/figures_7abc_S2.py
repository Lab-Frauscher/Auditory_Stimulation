...
3. Figures 7a-7c (Group Plots): The normalized IED count for the main cohort across three conditions:
   Figure 7a: IED count aligned to the arousal onset.
   Figure 7b: IED count aligned to the stimulation onset (for stimulations that
              resulted in arousal), overlaid with a line showing the percentage
              of active arousals.
   Figure 7c: IED count aligned to the stimulation onset for control stimulations.
4. Supplementary Figure 2: IED count aligned to arousal onset for each individual patient ID
...
fig_7a = pd.read_excel(file, sheet_name='Figure 7', header=4, usecols='B:C').rename(columns={
    'time relative to arousal onset (s, 500-ms bin centre)': 'time relative to arousal onset (s)'}).dropna(how='all')

fig_7b = pd.read_excel(file, sheet_name='Figure 7', header=4, usecols='E:F').rename(columns={
...
fig_7b_line = pd.read_excel(file, sheet_name='Figure 7', header=4, usecols='G:H').dropna(how='all')

fig_7c = pd.read_excel(file, sheet_name='Figure 7', header=4, usecols='J:K').rename(columns={
...
fig_s2 = pd.read_excel(file_supp, sheet_name='Supplementary Figure 2', header=3, usecols='B:D').rename(columns={
    'time relative to arousal onset (s, 500-ms bin centre)': 'time relative to arousal onset (s)'})

# bin length 500 ms
bin_len = 0.5

# Figure 7a: Bar Plot of IED Count During Arousals Relative to Arousal Onset
g = sns.catplot(
    data=fig_7a,
    kind="bar",
...
# Figure 7b: Bar Plot of IED Count During Arousals Relative to Stimulus Onset
g = sns.catplot(
    data=fig_7b,
    kind="bar",
...
# Figure 7c: Bar Plot of IED Count for Controls Relative to Stimulus Onset
g = sns.catplot(
    data=fig_7c,
    kind="bar",
    x="time relative to stimulus onset (s)",
...
# Supplementary Figure 2: Bar Plot of IED Count During Arousals Relative to Arousal Onset for Separated Patiens
g = sns.catplot(
    data=fig_s2,
    kind="bar",
    x="time relative to arousal onset (s)",
